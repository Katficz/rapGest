...
exports.raport_GET_list = function(req, res) {
    ...
    function(err, result) {
        ...
        var dates = []
        for(var i=4;i>0;i--){
            dates.push(startDate.getDate()+i + '-' + (startDate.getMonth()+1) + '-' + (startDate.getYear()+1900))
        }
        for(var i=1;i<5;i++){
            var match = false
            for(var j = 0;j<result.shiftC.length;j++)
            {
                if(result.shiftC[j].date.getDate() == startDate.getDate()+i) {
                    console.log('porownuje: ', result.shiftC[j].date.getDate(), ' z ', startDate.getDate()+i)
        ...
        res.render('raport-list-home', { title: 'Witaj! Wybierz raport', shiftA:shiftA, shiftB:shiftB, shiftC:shiftC, dates:dates} )
    }
    )
};
...
exports.raport_GET_update = function(req, res, next) {
    async.parallel({
        ...
        function(err, result) {
            ...
            var simpleYear = result.raport.date.getYear()+1900
            var simpleDay = result.raport.date.getDate()
            var simpleMonth = result.raport.date.getMonth()+1
            if(simpleMonth < 10) simpleMonth='0'+simpleMonth
            const simpleDate = simpleDay + '-' + simpleMonth + '-' + simpleYear
            res.render('raport-update', {title: 'Edytuj Raport zmiany ' + result.raport.shift + ' z dnia ' + simpleDate, data: result, absent: result.raport.teamAbsent, present:result.raport.teamPresent})
...
exports.raport_POST_deleteFailure = function(req, res) {
    ...
    .exec(function(err, result) {
        ...
        Failure.findByIdAndDelete(req.body.failureId)
        .exec(function(err, result) {
            if(err) {
                res.status(500).json(err)
                return next(err)
...
exports.raport_GET_myRaport = function(req, res, next) {
    ...
    startDate = new Date()
    endDate = new Date()
    if(shiftLoggedIn != 3 || (nowDate.getHours() > 6 && nowDate.getHours() < 24)){
        startDate.setHours(8,00)
        endDate.setHours(24)
        
        Raport.findOne({
            shift: shiftLoggedIn,
            date: { // searching for the same day between 0 - 24
                $gte: startDate,
                $lte: endDate,
            }
        })
    ...
    if(shiftLoggedIn == 3 && (6 > nowDate.getHours() && 0 < nowDate.getHours()) ) {
        ...
        Raport.findOne({
        shift: shiftLoggedIn,
        date: { // searching for the previous date 6 - 24
            $gte: startDate,
            $lte: nowDate,
        }
        })
